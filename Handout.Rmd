---
title: | 
  ![](Images/ClinicianCodersBranding_FINAL_CMYK_Colour.png){width=12cm}  
  "Handout"
output: pdf_document
...
Names are assigned to an object (e.g value/function/file). 

The assignment operator is `<-`.

You can then recall the value/function/file by using the object's name
...
You can also assign names to a text object, but note that the text needs to be surrounded with "". Also note that object names can't have spaces in them.
...
R is a statistical computing language; statistics uses vectors, matrices and data frames. 
...
You are unlikely to use these much, but it is good to know that they exist.
A list can contain elemnts of different types e.g. characters, numbers, vectors and even another list.
Lists are constructed using `list()`instead of `c()`.
```{r}
example_list <- list(name, age, gender, patients)
example_list
...
<p>
![Notice how there is no text in the source window to describe how you got to this data](Images/Pointandclick.png)
<p>
...
- You can therefore check an object before running a function on it. This would prevent you, for example, from trying to run a mean on a data frame.
...
2. Display the `discharge_dttm` vector in `cchic`.
...
## Addendum: File paths and "project-oriented workflows"

As you advance in your coding journey, you may will begin to run into some common issues, such as:
...
Fear not, there is a way around this, and it is sometimes known as using "project-oriented" workflows. While it is beyond the scope of this course to introduce this concept, we highly recommend taking a look at these resources:
...
- And for the You-Tubers among you, check out: [Improve your workflow for reproducible science](https://www.youtube.com/watch?v=JA-vLsN-sic&feature=youtu.be). This is a video from a 2-hour workshop on using R, R Markdown, Git, and GitHub to improve reproducibility. **It is an excellent primer for new starters on good practices to apply to every research project.** Slides from the workshop are available [here](https://mine-cetinkaya-rundel.github.io/improve-repro-workflow-reproducibilitea-2020/).
...
```{r}
cchic <- cchic %>% 
  mutate(discharge_day = day(ymd_hms(discharge_dttm)))
...
cchic_long %>%
  ...
  geom_boxplot()
```
What if you then want to convert back to original form?
...
- Scatterplots can reveal outliers that may not be obvious from looking at the data frame. 
...
# Workshop 6 - Control flow and looping

## Introduction

Two core principles that sit behind any programming language such as `R` are the use of control flow and loops. Control flow allows you to flexibly change what your code does depending on whether a condition is met, while loops allow you to execute code across many iterations. Together, these allow you to easily perform complex tasks tailored to your input, any number of times you need. 

## Control flow

In practice, control flow is often executed through `if` statements. A simple example of this is written in pseudo-code below: 

```{r example_if, eval = FALSE}

# can be read as, if condition_a is met
# then do_something
if(condition_a){
...
The above `if` statement has two core components:

1. The condition which must be a logical vector of length 1 (either TRUE/FALSE). When condition is TRUE, then the section inside the `{}` is executed. 
2. The `do_something()` above can be any bit of code that you would like to run. 

In reality, the vast majority of the time it's useful to create a conditional statement that evaluates to TRUE or FALSE. 
...
In the above, we create an example `weather` vector storing a string labeling the weather. The condition here is `weather == "rain"`, which will be evaluated into TRUE **only** if the `weather` vector matches "rain". In this case it does, which is why you see the "bring an umbrella!" printed, as the section in between the `{}` is evaluated. 

We can add multiple conditions using the `else if` syntax. For example if we wanted to categorise heights into "tall" and "short". 
...
You can have as many conditions as you want using `else if`, however sometimes it's easier to catch all exceptions to the conditions you've already set, rather than setting several more. This can be performed using an `else`. 
...
In practice, your data will probably sit inside a `data.frame` rather than in a vector. In which case you might want to use the function `ifelse`. `ifelse` is the **vectorised** version of an `if` statement. In fact, many of `R`s functions are vectorised for speed. What this allows you to do is pass a logical vector of **any** length (not just 1) into `ifelse`, and it will return a vector of the same length, with TRUE/FALSE converted into the values you set. 

```{r example_ifelse}

# initialize df object with heights from 100 to 200, incrementing by 20
df <- data.frame(height = seq(100, 200, 20))
df

# categorize heights into tall/not tall 
dplyr::mutate(df, height_cat = ifelse(test = height > 190, yes = "tall", no = "not tall"))

```
## Loops 

Loops are fundamental to any code. In reality, you've actually called many functions that have used loops behind the scenes already. Just, they are hidden by `R` or whoever designed the function, to make that function faster or more user friendly.

Let's start again with an example of a simple loop. 
...
1. `1:5` - a vector. This can be any object that permits iteration, but one of the simplest use cases is an integer vector shown. The loop will be iterating across this vector. The number of iterations of the loop is determined by the length of this vector and the value of `i` is determined by the contents of this vector. 
2. `i` - an index/iterator. This takes on the `i`th value of the vector at each iteration. E.g. in iteration 1, it's value takes on the 1st value of `1:5`. On iteration 2, the second value etc.  
3. `print(i)` - the contents of the loop. This can be any code you wish. For demonstration, this code prints the `i`. 

You can also loop across other vector types. 

```{r example_loop02}

animals <- c("cat", "dog", "monkey", "cow")
...
As the length of the `animals` vector is 4, the loop here has 4 iterations. During each iteration, `a` takes on the value of the vector `animals` at the position matching the iteration. Hence, why print `a` prints the 4 animals in the order they appear in the vector.
...
In this case, we've used the function `seq_along` to convert `animals` into an index vector. Then `i` becomes the index of each value in `animals`. And `animals[i]` uses `i` to extract the value at that position in `animals`. 

\newpage

# Workshop - Writing functions (deprecated)

This workshop has been deprecated as of Spring 2019 and is not taught as part of the course, however the materials are kept here for reference.
...
# Workshop - Basic Statistics (deprecated)

This workshop has been deprecated as of November 2020 and is not taught as part of the course, however the materials are kept here for reference. The reason for this is to cover more of the programming fundamentals during the course, rather than the applied statistics (which can vary greatly in their utility for each project). To meet this goal, we have added a new workshop "Control flow and looping" to replace this one. 
...
# ggplot only accepts data frames.
